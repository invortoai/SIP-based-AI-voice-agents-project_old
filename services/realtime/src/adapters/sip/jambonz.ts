...
export class JambonzBridge extends EventEmitter {
   ...
   */
  async connect(callInfo: SipCallInfo): Promise<void> {
   ...
   */
  private setupRtpSocket(): void {
    this.rtpSocket = dgram.createSocket('udp4');

    this.rtpSocket.on('message', (msg: Buffer, rinfo: dgram.RemoteInfo) => {
      // Parse RTP header
   ...
   * Connect to Jambonz WebSocket
   */
  private async connectWebSocket(callInfo: SipCallInfo): Promise<void> {
   ...
   */
  private sendAudioToSip(pcm16: Buffer): void {
   ...
   */
  private parseRtpHeader(packet: Buffer): RtpHeader {
    return {
   ...
   */
  private convertToPcm16(data: Buffer, codec: string): Buffer {
    switch (codec) {
      case 'PCMU':
   ...
   * Convert PCM16 to codec format
   */
  private convertFromPcm16(data: Buffer, codec: string): Buffer {
    switch (codec) {
      case 'PCMU':
   ...
   */
  private ulawToPcm16(ulaw: Buffer): Buffer {
   ...
   */
  private pcm16ToUlaw(pcm16: Buffer): Buffer {
   ...
   */
  private alawToPcm16(alaw: Buffer): Buffer {
   ...
   */
  private pcm16ToAlaw(pcm16: Buffer): Buffer {
   ...
   * μ-law decode table
   */
  private ulawDecode(ulawByte: number): number {
    const BIAS = 0x84;
    const CLIP = 32635;
   ...
   * A-law encode
   */
  private alawEncode(sample: number): number {
   ...
   */
  private async reconnect(callInfo: SipCallInfo): Promise<void> {
   ...
   * Transfer call
   */
  transferCall(destination: string, mode: 'blind' | 'attended' = 'blind'): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify({
        type: 'transfer',
        destination,
 ...
 */
export function createJambonzApp(config: JambonzAppConfig): any {
  return {
